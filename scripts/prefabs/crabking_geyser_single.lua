...
local geyserprefabs = {"crab_king_bubble1", "crab_king_bubble2", "crab_king_bubble3", "crab_king_waterspout"}
...
local function burble(inst)
    ...
    if TheWorld.Map:IsOceanAtPoint(x + offset.x, 0, z + offset.z) then
        ...
        if boat then ShakeAllCameras(CAMERASHAKE.VERTICAL, 0.1, 0.01, 0.3, boat, boat:GetPhysicsRadius(4)) end
    end
        ...
        if TheWorld.Map:IsOceanAtPoint(x + offset.x, 0, z + offset.z) then
            ...
            for _, v in ipairs(affected_entities) do
                if v.components.oceanfishable ~= nil then
                    ...
                    if projectile.components.weighable ~= nil then projectile.components.weighable.prefab_override_owner = inst.fisher_prefab end
                ...
                if v.components.health ~= nil and v.components.combat ~= nil and not v:HasTag("boat") and v.prefab ~= "crabking_claw" and v.prefab ~= "crabking" then v.components.combat:GetAttacked(inst.crab, 34 + inst.crab.countgems(inst.crab).purple * 2) end
            end
        else
            local boat = TheWorld.Map:GetPlatformAtPoint(x + offset.x, z + offset.z)
            if boat then
                ...
                if ents and #ents > 0 then for i, ent in ipairs(ents) do if ent:GetCurrentPlatform() and ent:GetCurrentPlatform() == inst then if ent:HasTag("boat_health_buffer") then mult = 0.33 end end end end

                boat.components.health:DoDelta((-2 - math.floor(inst.crab.countgems(inst.crab).purple * 0.5)) * mult)
                boat.SoundEmitter:PlaySoundWithParams("turnoftides/common/together/boat/damage", {intensity = .6})
...
local function geyserfn()
    ...
    inst:DoTaskInTime(TUNING.CRABKING_CAST_TIME + 2, function() inst:Remove() end)
