...
Crab King Rework: New attack pattern, mechanics, and more!
...
local function BinaryConfig(name, label, hover, default) return {name = name, label = label, hover = hover, options = {{description = "Enabled", data = true}, {description = "Disabled", data = false}}, default = default} end
...
local function MultiplierConfig(name, label, hover, min, max, step, default)
    ...
    for i = min or 1, max, step or 1 do return_table.options[#return_table.options + 1] = {description = i .. "x", data = i} end
    return return_table
end

local function PercentConfig(name, label, hover, min, max, step, default)
    local return_table = {name = name, label = label, hover = hover, options = {}, default = default}
    for i = min or 0.1, max + step, step or 0.1 do return_table.options[#return_table.options + 1] = {description = 100 * i .. "%", data = i} end
    return return_table
end
...
configuration_options = {
    ...
    BinaryConfig("config_turningBoats", "Steering Wheel Turning", "Steering Wheels will also rotate the boat if it has a rudder.", false),
    {
        ...
        options = {{description = "33% (Default)", data = 0.66}, {description = "66%", data = 0.33}, {description = "100%", data = 0.00}},
        default = 0.33,
    ...
    },
    {
        ...
        options = {{description = "Often", data = 0.75}, {description = "Default", data = 0.50}, {description = "Rare", data = 0.33}, {description = "Disabled", data = 0}},
        default = 0.50,
        hover = "Choose how common custom treasure presets are."
    },
    {name = "config_cheapBoats", label = "Boat Cost", options = {{description = "Default", data = 0}, {description = "Cheap", data = 1}, {description = "Cheapest", data = 2}}, default = 1, hover = "Choose how cheap you want boat parts to be."},
