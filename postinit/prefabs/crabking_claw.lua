...
if env.GetModConfigData("config_crabkingRework") and not UPDATE_CHECK then
    local function BonusDamage(inst)
        ...
        if #ents > 0 then for i, v in ipairs(ents) do if v:IsValid() and not v:IsInLimbo() then if v.components.health ~= nil and not v.components.health:IsDead() then v.components.health:DoDelta(-inst.components.health.maxhealth) end end end end
    end
    ...
    local function clamp(inst)
        if inst.boat and not inst.boat.components.health:IsDead() then
            ...
            if _ents and #_ents > 0 then for i, ent in ipairs(_ents) do if ent:GetCurrentPlatform() and ent:GetCurrentPlatform() == inst then if ent:HasTag("boat_health_buffer") then mult = 0.33 end end end end
            ...
            for i, v in pairs(ents) do
                if v ~= inst and v:IsValid() and not v:IsInLimbo() then if v.components.health ~= nil and not v.components.health:IsDead() and inst.components.combat:CanTarget(v) then inst.components.combat:DoAttack(v) end end
            end

            ShakeAllCameras(CAMERASHAKE.VERTICAL, 0.3, 0.03, 0.5, inst.boat, inst.boat:GetPhysicsRadius(4))

            if inst.boat.components.boatphysics ~= nil then inst.boat.components.boatphysics:AddBoatDrag(inst) end
