...
local function getoffsetfromtreasurespawnpoint(point_ind, radius, attempts, doer)
    ...
    if doer ~= nil and doer:GetDistanceSqToPoint(x, y, z) <= DOER_CHECK_RADIUS_SQ then return nil end

    -- If a point was found a check is also made to make sure it's not right next to land
    if FindSwimmableOffset(Vector3(x, y, z), 0, SHORE_CHECK_RADIUS, SHORE_CHECK_ATTEMPTS) == nil then return nil end

    for _, v in ipairs(AllPlayers) do if v:GetDistanceSqToPoint(x, y, z) <= ALLPLAYERS_CHECK_RADIUS_SQ then return nil end end
...
local function gettreasurepos(doer)
    ...
    if treasure_spawn_positions == nil then treasure_spawn_positions = {} end
    ...
    if not offset or not base_point then return false, "NO_VALID_SPAWN_POINT_FOUND" end

    return Vector3(base_point.x + offset.x, 0, base_point.z + offset.z)
end
...
local function AddMinimapMarker(treasure, data)
    if data.underwater_object ~= nil then if data.underwater_object.components.treasuremarked ~= nil then data.underwater_object.components.treasuremarked:TurnOn() end end
        ...
        if not is_not_from_hermit and hermitcrab ~= nil and not self:GetPlayerHasFoundHermit(doer) then
            ...
            if num_active_hunts < MAX_ACTIVE_TREASURE_HUNTS then
                ...
                for i = 1, MAX_ACTIVE_TREASURE_HUNTS do
                    ...
                    local keys = {}
                    for k, v in pairs(self.active_treasure_hunt_markers) do table.insert(keys, k) end
