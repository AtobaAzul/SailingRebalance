...
local function DoLineAttack(inst, target, position)
    ...
    for _ = 1, 3 do
        ...
        for i = 1, math.clamp(dist, 0, 16) do
            inst:DoTaskInTime(FRAMES * i, function()
                ...
                local affected_entities = TheSim:FindEntities(dx, dy, dz, TUNING.TRIDENT.SPELL.RADIUS, MUST_HAVE_SPELL_TAGS, CANT_HAVE_SPELL_TAGS, MUST_HAVE_ONE_OF_SPELL_TAGS)
                for _, v in ipairs(affected_entities) do if v.prefab ~= "boat" or TheNet:GetPVPEnabled() then inst:DoWaterExplosionEffect(v, owner, position) end end
                ...
                if TheWorld.Map:IsOceanTileAtPoint(dx, dy, dz) and not TheWorld.Map:IsVisualGroundAtPoint(dx, dy, dz) then
                    ...
                    if platform_at_point ~= nil then
                        platform_at_point.SoundEmitter:PlaySoundWithParams("turnoftides/common/together/boat/damage", {intensity = .3})
                    else
    ...
    for _, v in pairs(TheSim:FindEntities(x, y, z, TUNING.TRIDENT_FARM_PLANT_INTERACT_RANGE, PLANT_TAGS)) do if v.components.farmplanttendable ~= nil then v.components.farmplanttendable:TendTo(owner) end end
...
local function OnAttack(inst, attacker, target)
    ...
    if inst.charge > 3 then
        ...
        local fx = TheWorld.Map:IsOceanTileAtPoint(x, y, z) and not TheWorld.Map:IsVisualGroundAtPoint(x, y, z) and TheWorld.Map:GetPlatformAtPoint(x, z) == nil and SpawnPrefab("crab_king_waterspout") or SpawnPrefab("trident_ground_fx")
...
if env.AddPrefabPostInit("config_tridentRework") then
    ...
    env.AddPrefabPostInit("trident", function(inst)
        ...
        if inst.components.spellcaster ~= nil then inst.components.spellcaster:SetSpellFn(DoLineAttack) end
